...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
WARMAGE_BINARY = maged
WARMAGE_DIR = warmage
BUILDDIR ?= $(CURDIR)/build
BUILD_BRIDGING_DIR ?= $(CURDIR)/build-bridging
SIMAPP = ./app
HTTPS_GIT := https://github.com/petri-labs/warmage.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := warmagezone
PROJECT := warmage
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=warmage \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(WARMAGE_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm warmage || true
	# create a new container from the latest image
	$(DOCKER) create --name warmage -t -i ${DOCKER_IMAGE}:latest warmage
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp warmage:/usr/bin/maged ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/petri-labs/warmage/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(WARMAGE_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(WARMAGE_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(WARMAGE_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(WARMAGE_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/petri-labs/warmage
.PHONY: format
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\maged:warmage\Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\maged:warmage\Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\maged:warmage\Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\maged:warmage\Z maged/node "./maged unsafe-reset-all --home=/warmage"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/maged:/warmage:Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/maged:/warmage:Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/maged:/warmage:Z maged/node "./maged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/maged:/warmage:Z maged/node "./maged unsafe-reset-all --home=/warmage"
endif
...
bridging-localnet-start: bridging-localnet-stop build-linux localnet-build
	sudo mkdir -p $(BUILD_BRIDGING_DIR) && sudo cp -r $(BUILDDIR)/$(WARMAGE_BINARY) $(BUILD_BRIDGING_DIR)/
	@if ! [ -f $(BUILD_BRIDGING_DIR)/node0/maged/config/genesis.json ]; \
	then docker run --rm -v $(BUILD_BRIDGING_DIR):/warmage:Z warmagezone/localnetnode testnet init-files -v 4 -o /warmage --starting-ip-address 192.168.11.2 --predetermined-mnemonic --keyring-backend=test; \
	fi
...
PACKAGE_NAME:=github.com/petri-labs/warmage
